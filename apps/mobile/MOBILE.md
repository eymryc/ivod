...
- Pages dédiées : Films, Séries, Web-séries, Animation (`/catalog/[type]`)
- Explorer / browse (filtres type + tri + recherche)
- Recherche + filtres
- Tarifs publics (`/pricing`) — masqué du menu si connecté
- Recommandations (`/recommendations`) — connecté
...
### Auth & profils
- **Navigation invité** : catalogue, fiches, tarifs, recherche sans compte
- Login MDP / OTP (inline), register, forgot / reset / setup password
...
│   └── notifications.tsx
├── src/
│   ├── core/               # entités, monetization, catalog, utils
│   ├── infrastructure/     # API modules, services (offline, push)
│   ├── presentation/       # hooks, utils UI
│   ├── components/         # UI viewer (content, player, pricing…)
│   ├── store/              # auth, profil, toasts
│   └── theme/              # couleurs, typographie, layout
└── MOBILE.md
```
...
## Hors scope mobile (web uniquement ou non livré)

- **Admin** (`/admin/*`) — back-office modération, rails, paiements admin
- **Studio** (`/studio/*`) — upload vidéo, pipeline, analytics créateur
- **Live streaming** — retiré (module API supprimé) ; non prévu sur mobile viewer
- Téléchargement HLS segment-par-segment complet (offline = métadonnées + fichier local)
- Deep link automatique retour Paystack (écran `/payment/callback` présent ; ouverture navigateur manuelle)
