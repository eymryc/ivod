...
export const watchApi = {
  ...
  /** Alias compat écrans legacy */
  history: (profileId?: string, page = 1, limit = 20): Promise<WatchHistoryResult> => {
    if (profileId) {
      return api.get<WatchHistoryResult>(
        `/watch-sessions/history/profile/${profileId}${buildQueryString({ page, limit })}`,
      );
  ...
  }) => api.post<{ id: string; recorded: boolean }>('/watch-sessions/qoe', data),
};
