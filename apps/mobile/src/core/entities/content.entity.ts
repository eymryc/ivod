...
export interface Episode {
  ...
  title: string;
  /** Clé objet MinIO du thumbnail (résoudre via mediaUrl). */
...
export function getContentPosterUrl(
  content: Pick<Content, 'posterUrl' | 'thumbnailUrl' | 'mediaAssets'>,
  resolveMedia?: (key?: string | null) => string | undefined,
): string | null {
  if (content.posterUrl) return content.posterUrl;
  if (content.thumbnailUrl) return content.thumbnailUrl;
  const posterKey = content.mediaAssets?.find((a) => a.type?.code === 'POSTER')?.objectKey;
  if (posterKey && resolveMedia) return resolveMedia(posterKey) ?? null;
  return null;
}

/** Bannière hero (asset BANNER) ou fallback poster/thumbnail. */
export function getContentBannerUrl(
  content: Pick<Content, 'posterUrl' | 'thumbnailUrl' | 'mediaAssets'>,
  resolveMedia?: (key?: string | null) => string | undefined,
): string | null {
  const bannerKey = content.mediaAssets?.find((a) => a.type?.code === 'BANNER')?.objectKey;
  if (bannerKey && resolveMedia) {
    return resolveMedia(bannerKey) ?? null;
  }
