...
async function seedUserSubscriptions() {
  ...
  for (const seedUser of SEED_USERS) {
    ...
    const provider = await prisma.refPaymentProvider.findUnique({
      where: { code: seedUser.subscription.providerCode ?? 'PAYSTACK' },
      select: { id: true },
    });
...
async function main() {
  ...
  console.log('\n🎬 Rails catalogue éditoriaux');
  const { seedCatalogRailsDb } = await import(
    '../src/modules/catalog/config/editorial-rails.seed'
  );
  await seedCatalogRailsDb(prisma);
  const railCount = await prisma.editorialRail.count();
  console.log(`  ✓ editorial_rails: ${railCount}`);
