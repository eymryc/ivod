...
export class CommentsService {
  ...
  async create(userId: string, contentId: string, body: string, parentId?: string) {
    ...
    // Notify asynchronously — never block the response
    this.dispatchCommentNotification({ userId, contentId, commentId: comment.id, parentId }).catch(
      (err: Error) => this.logger.error('Erreur notification commentaire', err.message),
    );
  ...
  private async dispatchCommentNotification(opts: {
    userId: string; contentId: string; commentId: string; parentId?: string;
  }) {
    if (opts.parentId) {
      // Notify the parent comment author (COMMENT_REPLY)
      const parent = await this.prisma.comment.findUnique({
        where: { id: opts.parentId },
        include: { profile: { select: { userId: true } } },
      });
      const targetUserId = parent?.profile?.userId;
      if (targetUserId && targetUserId !== opts.userId) {
        await this.notifications.dispatch({
          userId: targetUserId,
          type: NotificationType.COMMENT_REPLY,
          title: 'Nouvelle réponse à votre commentaire',
          body: 'Quelqu\'un a répondu à votre commentaire.',
          data: { contentId: opts.contentId, commentId: opts.commentId },
        });
      }
    } else {
      // Notify the content creator (NEW_COMMENT)
      const content = await this.prisma.content.findUnique({
        where: { id: opts.contentId },
        select: { uploadedByUserId: true },
      });
      const targetUserId = content?.uploadedByUserId;
      if (targetUserId && targetUserId !== opts.userId) {
        await this.notifications.dispatch({
          userId: targetUserId,
