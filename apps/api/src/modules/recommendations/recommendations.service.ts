...
import {
  CATALOG_CARD_CONTENT_INCLUDE,
  mapContentToCatalogCard,
} from '../../common/helpers/content-card.mapper';
...
export class RecommendationsService {
  ...
  private async compute(profileId: string, limit: number, userId?: string) {
    ...
    let candidates = await this.prisma.content.findMany({
      ...
      include: CATALOG_CARD_CONTENT_INCLUDE,
    });
    ...
    return {
      items: scored.map((c) => mapContentToCatalogCard(c)),
  ...
  private async getTrending(limit: number) {
    ...
    return {
      items: contents.map((c) => mapContentToCatalogCard(c)),
      personalized: false,
    };
  }
}
