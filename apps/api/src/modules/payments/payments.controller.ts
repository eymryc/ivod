...
export class PaymentsController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('BearerAuth')
  @ApiOperation({ summary: 'Initier un paiement via Paystack (carte ou Mobile Money)' })
  ...
  devComplete(@Param('id') id: string) {
    // Délègue à la même méthode que le service (source de vérité unique —
    // voir PaymentsService.allowPaymentSimulation) plutôt que de dupliquer
    // une condition NODE_ENV séparée et potentiellement divergente.
    if (!this.service.allowPaymentSimulation()) {
