...
export class VideoPipelineNotificationService {
  ...
  async notifyPreviewReady(assetId: string): Promise<void> {
    ...
    if (ctx.userId) {
      ...
      if (user?.email) {
        this.mail.sendVideoReadyEmail({
          to: user.email,
          creatorFirstName: user.firstName ?? user.email,
          contentTitle: ctx.contentTitle,
          episodeLabel: ctx.episodeLabel,
        }).catch((err: Error) => this.logger.error('Erreur email aperçu vidéo', err.message));
      }
    }
  }
    ...
    if (ctx.userId) {
      ...
      if (user?.email) {
        this.mail.sendVideoReadyEmail({
          ...
          contentTitle: ctx.contentTitle,
          episodeLabel: ctx.episodeLabel,
        }).catch((err: Error) => this.logger.error('Erreur email vidéo prête', err.message));
      }
    }
  }
