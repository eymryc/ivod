...
export class VideosController {
  ...
  @HttpCode(200)
  @ApiOperation({ summary: 'Relancer un encodage en échec (créateur)' })
  @ApiParam({ name: 'assetId' })
  retryFailedPipeline(
    @CurrentUser('id') userId: string,
    @Param('assetId') assetId: string,
  ) {
    return this.videosService.retryFailedPipeline(assetId, userId);
  }
  ...
  getEpisodeStream(
    ...
    return this.videosService.getSignedPlaybackUrlForEpisode(episodeId, user.id, user.roles, req);
  }
  ...
  getStream(
    ...
    return this.videosService.getSignedPlaybackUrl(contentId, user.id, user.roles, req);
  }
