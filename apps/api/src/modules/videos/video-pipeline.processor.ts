...
interface TranscodePayload { assetId: string; sourceKey: string; meta: SourceMetadata }
...
export class VideoPipelineProcessor extends WorkerHost {
  ...
  private async handleProbe(job: Job<ProbePayload>): Promise<void> {
    ...
    try {
      ...
      // Download source preserving original extension (MOV, MKV, AVI…)
      ...
      if (isTwoPhasePipelineEnabled()) {
        await this.pipeline.enqueueTranscodePreview(assetId, asset.sourceObjectKey, meta);
        this.logger.log(`PROBE done [${assetId}] — preview 720p en file`);
      } else {
        await this.pipeline.enqueueTranscode(assetId, asset.sourceObjectKey, meta);
        this.logger.log(`PROBE done [${assetId}] — transcodage complet en file`);
      }
      ...
      await this.failAsset(assetId, videoJob.id, err);
      throw err;
    } finally {
  ...
  private async handleTranscodePreview(job: Job<TranscodePayload>): Promise<void> {
    const { assetId, sourceKey } = job.data;
    const tmpDir = this.tmpDir(assetId);
    ...
    try {
      ...
      await this.doneVideoJob(videoJob.id);
      await this.pipeline.enqueuePackage(assetId, 'preview');
  ...
  /** Phase 3b — ladder complète (profils restants) */
  ...
  private async prepareTranscode(
    ...
    const ext = path.extname(sourceKey).replace('.', '') || 'mp4';
    const sourceFile = path.join(tmpDir, `source.${ext}`);
    await this.minio.downloadFile(this.minio.bucketVideos, sourceKey, sourceFile);
  ...
  private async transcodeRendition(
    ...
    const ffmpegArgs = [
      ...
      '-threads', '0',           // décodage multi-thread de la source
      '-i',       sourceFile,
  ...
  private async handlePackage(job: Job<PackagePayload>): Promise<void> {
    const { assetId } = job.data;
    ...
    try {
      ...
      const manifestKey = await this.buildAndUploadMasterManifest(assetId, renditions);
      ...
      if (mode === 'preview') {
        ...
        await this.pipeline.enqueueTranscodeFull(
          assetId,
          asset.sourceObjectKey,
          await this.loadMetaForAsset(asset, renditions),
        );
        return;
      }

      await this.pipeline.enqueueThumbnail(assetId);
  ...
  /** Reconstruit meta minimale pour TRANSCODE_FULL après preview */
  private async loadMetaForAsset(
    asset: { durationSec: number | null; width: number | null; height: number | null; frameRate: number | null },
    _renditions: unknown[],
    ...
    return {
      ...
      audioChannels: 0,
      formatName: 'unknown',
      displayWidth: asset.width ?? 0,
      displayHeight: asset.height ?? 0,
    };
  }
  ...
  private async failAsset(assetId: string, videoJobId: string, err: unknown): Promise<void> {
    const message = err instanceof Error ? err.message : String(err);
    this.logger.error(`Pipeline FAILED [${assetId}]: ${message}`);
    await Promise.all([
