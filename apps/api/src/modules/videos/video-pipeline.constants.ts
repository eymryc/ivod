...
/** Verrou BullMQ par défaut — 4h couvre les longs films ; auto-renew maintient le lock actif. */
const DEFAULT_JOB_LOCK_MS = 14_400_000;
const DEFAULT_JOB_LOCK_RENEW_MS = 300_000;

/** Parallélisme max d'uploads MinIO par rendition (évite les 429 sur gros fichiers HLS). */
export const MINIO_UPLOAD_CONCURRENCY = 20;
...
/**
 * Threads FFmpeg par job (décodage + encodage libx264).
...
/**
 * Priorité BullMQ selon le plan actif (inférieur = plus prioritaire).
 * PREMIUM → 1, BASIC → 5, FREE → 10 (tous les autres plans)
 */
export function resolvePriorityForPlan(planCode: string): number {
  if (planCode === 'PREMIUM') return 1;
  if (planCode === 'BASIC')   return 5;
  return 10;
}
