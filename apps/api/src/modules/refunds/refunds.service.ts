...
export class RefundsService {
  ...
  async request(userId: string, paymentId: string, reason?: string) {
    ...
    this.notifications.dispatch({
      ...
      body: `Votre demande de remboursement de ${payment.amount} ${payment.currency} a été enregistrée.`,
      data: { refundId: refund.id, paymentId, amount: Number(payment.amount), currency: payment.currency },
    }).catch((err: Error) => this.logger.error('Notification remboursement demandé', err.message));
      ...
      if (refund.payment?.id) {
        ...
        if (payment?.user) {
          ...
          this.notifications.dispatch({
            ...
            body: `Votre demande de remboursement de ${refund.amount} ${refund.currency} a été rejetée.`,
            data: { refundId: refund.id, paymentId: refund.paymentId, amount: Number(refund.amount), currency: refund.currency },
          }).catch((err: Error) => this.logger.error('Notification remboursement rejeté', err.message));
        }
      }
    }
  ...
  private async finalizeRefund(
    ...
    await this.notifications
      .dispatch({
        ...
        body: `Votre remboursement de ${payment.amount} ${payment.currency} a été traité avec succès.`,
        data: { refundId, paymentId, amount: Number(payment.amount), currency: payment.currency },
      })
