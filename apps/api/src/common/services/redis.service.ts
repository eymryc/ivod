...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  /**
   * Verrou distribué best-effort (SET NX PX) — utilisé pour qu'un job planifié
   * (@Cron) ne s'exécute qu'une fois même si plusieurs réplicas API tournent
   * en parallèle (voir CronService). Pas de libération garantie par jeton
   * (pas de script Lua compare-and-delete type Redlock) : suffisant ici car
   * les jobs sont peu fréquents (horaire/quotidien) et le TTL expire de
   * toute façon en cas de crash — un vrai Redlock serait nécessaire pour un
   * verrou à forte contention ou à sémantique critique.
   */
  async acquireLock(key: string, ttlSec: number): Promise<boolean> {
    try {
      const result = await this.client.set(`lock:${key}`, '1', 'EX', ttlSec, 'NX');
      return result === 'OK';
    } catch {
      // Panne Redis : on choisit de laisser le job s'exécuter plutôt que de
      // bloquer silencieusement toutes les tâches planifiées.
      return true;
    }
  }

  async releaseLock(key: string): Promise<void> {
    try { await this.client.del(`lock:${key}`); } catch { /* expire naturellement via TTL */ }
  }
