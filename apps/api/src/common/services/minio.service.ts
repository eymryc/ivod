...
export class MinioService implements OnModuleInit {
  ...
  constructor(private config: ConfigService) {
    ...
    this.urlClient = new Minio.Client({
      ...
      accessKey,
      secretKey,
      region:   'us-east-1', // évite l'appel HTTP getBucketRegion lors de presignedPutObject/GetObject
    });
  }
  ...
  publicUrl(bucket: string, objectKey: string): string {
    const proto = this.config.get('MINIO_USE_SSL', 'false') === 'true' ? 'https' : 'http';
